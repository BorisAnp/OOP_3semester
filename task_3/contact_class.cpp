...
std::atomic<long long> Contact::s_constructed{0};
std::atomic<long long> Contact::s_destroyed{0};
std::atomic<long long> Contact::s_copyCtor{0};
std::atomic<long long> Contact::s_moveCtor{0};
std::atomic<long long> Contact::s_copyAssign{0};
std::atomic<long long> Contact::s_moveAssign{0};
std::atomic<long long> Contact::s_newCalls{0};
std::atomic<long long> Contact::s_deleteCalls{0};
...
{
    s_constructed.fetch_add(1, std::memory_order_relaxed);
    s_copyCtor.fetch_add(1, std::memory_order_relaxed);
}

Contact::Contact(Contact&& other) noexcept
    : name_(std::move(other.name_)),
    surname_(std::move(other.surname_)),
    patronymic_(std::move(other.patronymic_)),
    email_(std::move(other.email_)),
    address_(std::move(other.address_)),
    birth_date_(other.birth_date_),
    phones_(std::move(other.phones_))
{
    s_constructed.fetch_add(1, std::memory_order_relaxed);
    s_moveCtor.fetch_add(1, std::memory_order_relaxed);
}
...
Contact& Contact::operator=(const Contact& other) {
    ...
    s_copyAssign.fetch_add(1, std::memory_order_relaxed);
    return *this;
}
...
Contact& Contact::operator=(Contact&& other) noexcept {
    ...
    s_moveAssign.fetch_add(1, std::memory_order_relaxed);
    return *this;
}
...
void* Contact::operator new(std::size_t sz) {
    s_newCalls.fetch_add(1, std::memory_order_relaxed);
    return ::operator new(sz);
}

void Contact::operator delete(void* p) noexcept {
    s_deleteCalls.fetch_add(1, std::memory_order_relaxed);
    ::operator delete(p);
}

Contact::Stats Contact::stats() {
    Stats st;
    st.constructed = s_constructed.load(std::memory_order_relaxed);
    st.destroyed   = s_destroyed.load(std::memory_order_relaxed);
    st.copyCtor    = s_copyCtor.load(std::memory_order_relaxed);
    st.moveCtor    = s_moveCtor.load(std::memory_order_relaxed);
    st.copyAssign  = s_copyAssign.load(std::memory_order_relaxed);
    st.moveAssign  = s_moveAssign.load(std::memory_order_relaxed);
    st.newCalls    = s_newCalls.load(std::memory_order_relaxed);
    st.deleteCalls = s_deleteCalls.load(std::memory_order_relaxed);
    return st;
}

void Contact::resetStats() {
    s_constructed.store(0);
    s_destroyed.store(0);
    s_copyCtor.store(0);
    s_moveCtor.store(0);
    s_copyAssign.store(0);
    s_moveAssign.store(0);
    s_newCalls.store(0);
    s_deleteCalls.store(0);
}
